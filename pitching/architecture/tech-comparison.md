...
## Wallet Integration: Why RainbowKit?

Wallet connectivity is often the biggest barrier to Web3 adoption. We use **RainbowKit** for wallet connections and **Google OAuth 2.0** for social authentication.

### Comparison

Here's how popular wallet integration solutions compare:

| Feature                  | RainbowKit  | Web3Modal | Thirdweb  | WalletConnect |
| ------------------------ | ----------- | --------- | --------- | ------------- |
| **Social Login**         | Via Backend | Yes (v3)  | Yes       | No            |
| **Wallet Support**       | 100+        | 100+      | 50+       | 100+          |
| **UI Customization**     | High        | Medium    | High      | Low           |
| **Developer Experience** | Excellent   | Good      | Good      | Good          |
| **Pricing**              | Free        | Free      | Free tier | Free          |
| **Built for Base**       | Yes         | Yes       | Yes       | Yes           |

**Our Approach:**

- **Google OAuth 2.0**: Server-side authentication via Passport.js for social login
- **RainbowKit**: For Web3 wallet connections (MetaMask, Coinbase Wallet, WalletConnect, etc)
- **Wallet Sync**: Users can link their Web3 wallet to their Google account

**Why This Architecture?**

- **Separation of Concerns**: Authentication (backend) vs wallet connection (frontend)
- **Security**: OAuth handled server-side with secure JWT cookies
- **Flexibility**: Users can login with Google, wallet, or both
- **Better UX**: No need for embedded wallets or complex account abstraction
...
| **Authentication**  | Google OAuth 2.0 + RainbowKit | Dual auth for Web2 & Web3 users |
