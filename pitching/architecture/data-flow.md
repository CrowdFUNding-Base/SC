...
```mermaid
flowchart TB
    subgraph Layer1["Layer 1: Blockchain (Source of Truth)"]
        Contracts["Smart Contracts"]
        Events["Event Logs"]
    end

    subgraph Layer2["Layer 2: Indexer (Real-time Processing)"]
        Ponder["Ponder Indexer"]
        IndexerDB["Indexed Database"]
    end

    subgraph Layer3["Layer 3: Backend Cache (Fast Queries)"]
...
```mermaid
flowchart TB
    subgraph Blockchain["Blockchain Events"]
        E1["CampaignCreated"]
        E2["DonationReceived"]
        E3["FundWithdrawn"]
        E4["BadgeMinted"]
...
sequenceDiagram
    ...
    Frontend->>User: Display new badge
```
